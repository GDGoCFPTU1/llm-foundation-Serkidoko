...
def call_anthropic(
    ...
    if text is None:
        raise RuntimeError("Unable to parse Anthropic response text.")

    usage = {
        "input_tokens": getattr(response.usage, "input_tokens", None),
        "output_tokens": getattr(response.usage, "output_tokens", None),
    }
    return text, latency_seconds, usage
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        for key, stats in result.items():
            if key == "prompt":
                continue
            model_label = model_names.get(key, key)
            response = str(stats.get("response", ""))
            response = response.replace("\n", " ")
            if len(response) > 50:
                response = response[:47] + "..."
            latency = stats.get("latency", "")
            tokens = f"{stats.get('input_tokens', '')}/{stats.get('output_tokens', '')}"
            cost = stats.get("cost", "")
            rows.append(
